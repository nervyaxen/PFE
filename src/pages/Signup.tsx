...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else setError(t("auth.emailTaken"));
  };
      ...
      >
        <div className="glass-panel rounded-2xl p-8 border border-border/50">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold font-heading">{t("auth.signupTitle")}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-1.5 block">{t("auth.name")}</label>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            {t("auth.hasAccount")}{" "}
            <Link to="/login" className="text-neon hover:underline">{t("auth.loginLink")}</Link>
          </p>
        </div>
      </motion.div>
    </div>
