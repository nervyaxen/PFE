...
const Pricing = () => {
  ...
  const goToPayment = (planKey: string) => {
    navigate("/payment", { state: { plan: planKey, yearly } });
  };
        ...
        >
          ...
          <div className="inline-flex items-center gap-3 glass-panel rounded-full p-1">
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${!yearly ? "bg-neon text-accent-foreground" : "text-muted-foreground"
                }`}
            >
              {t("pricing.monthly")}
            </button>
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <motion.div
                ...
                className={`relative glass-panel rounded-2xl p-8 flex flex-col ${plan.featured
